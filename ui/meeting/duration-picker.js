...
(function ($) {
    ...
    $.fn.durationPicker = function(options){
        if (options == undefined) {
            ...
            var settings = $.extend(true, {}, {classname: '', responsive: true, type:'number'}, options);
        }
    ...
    function generate_template (settings, stages) {
        var html = '<span class="durationpicker-container ' + settings.classname + '">';
        var type = settings.type;
        for (var item in stages){
            html += '<span class="durationpicker-innercontainer"><input min="' + settings[stages[item]]['min'] + '" max="' + settings[stages[item]]['max'] + '" placeholder="0" type="' + type + '" id="duration-' + stages[item] + '" class="durationpicker-duration" ><span class="durationpicker-label">' + settings[stages[item]]['label'] + '</span></div>';
        }
        html += '</span>';
